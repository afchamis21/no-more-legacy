    ...
    : OpenAiClient<ConversionRequest, ConversionResponse>(configuration, logger), IFileConversor
{
    ...
    protected override string SystemPrompt() => 
        ...
        Primary Objective: To deconstruct a JSF `ManagedBean` and its `.xhtml` view into two distinct parts: 1) a stateless Spring Boot 6 REST API backend and 2) a modern Angular 18+ standalone component for the frontend.
        
        Context: This conversion requires separating concerns. You will receive a `ManagedBean` and its view, along with a `Context` object, including library migration suggestions. Your task is to create two sets of files: one for the Spring Boot API and one for the Angular component, using modern libraries as specified.
        
        ## Detailed Instructions:
        1.  Analyze the legacy `Files` (`.java` and `.xhtml`) and the provided `Context`.
        2.  **Library Migration**: Analyze the `Libraries` array in the `Context`. In the generated code, you **must** replace any usage of the `Old` library with its `New` suggested equivalent. This applies to both stacks (e.g., replace PrimeFaces with Angular Material in the frontend, and `java.util.Date` with `java.time` in the backend).
        3.  **For the Backend (Spring Boot):**
            ...
            * Create a new Spring `@RestController` to expose this logic as RESTful endpoints. Data properties should be exposed via `GET` endpoints. Action methods should become `POST` or `PUT` endpoints.
            * Create DTOs for any data structures used by the new endpoints, using modern types as indicated by the library migration context.
        4.  **For the Frontend (Angular 18+):**
        ...
        5.  **Reliability Rule**: If you encounter complex JSF lifecycle events, view-scoped logic, or custom components that are hard to decouple, you **must add an explanatory comment** in both the backend and frontend code. The comment must follow the format: `// TODO: [AI-CONFIDENCE: LOW] Your explanation here.`
